...
export function WhatsAppFAB() {
    ...
    const [bottomOffset, setBottomOffset] = useState("7.5rem");

    useEffect(() => {
        const handleTabBar = (e: CustomEvent) => {
            setBottomOffset(e.detail.visible ? "7.5rem" : "1.5rem");
        };
    ...
    return (
        <div className="fixed right-5 z-50 flex flex-col items-end md:!bottom-6 transition-[bottom] duration-300" style={{ bottom: bottomOffset }}>
            <AnimatePresence>
                {/* Expanded Card */}
                {isExpanded && (
                    <motion.div
            ...
            <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="relative flex items-center justify-center w-12 h-12 bg-[#25D366] text-white rounded-full shadow-lg hover:shadow-2xl hover:shadow-[#25D366]/40 transition-all duration-300 z-10"
